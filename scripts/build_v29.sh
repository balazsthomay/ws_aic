#!/usr/bin/env bash
# v29: OnlineIK + joint-space wiggle-on-contact seat. Reuses v26 localizer weights.
set -euo pipefail
...
TAG="${TAG:-v29}"
DOCKERFILE="${DOCKERFILE:-$SRC/docker/aic_model/Dockerfile.v29}"
...
if [[ ! -f "$WEIGHTS_DST/port_localizer_dinov2.pt" || ! -f "$WEIGHTS_DST/port_localizer_stats.npz" ]]; then
  ...
  echo "  cp data/outputs/loc_v29.pt $WEIGHTS_DST/port_localizer_dinov2.pt" >&2
  echo "  cp data/outputs/loc_v29_stats.npz $WEIGHTS_DST/port_localizer_stats.npz" >&2
  exit 1
fi

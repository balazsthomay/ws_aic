#!/usr/bin/env bash
# v27: OnlineIK + force-feedback seat. Reuses v26 localizer weights.
set -euo pipefail
...
TAG="${TAG:-v27}"
DOCKERFILE="${DOCKERFILE:-$SRC/docker/aic_model/Dockerfile.v27}"
...
if [[ ! -f "$WEIGHTS_DST/port_localizer_dinov2.pt" || ! -f "$WEIGHTS_DST/port_localizer_stats.npz" ]]; then
  echo "Missing localizer weights at $WEIGHTS_DST (re-stage from v26 build)" >&2
  echo "  cp data/outputs/loc_dinov3_gz_v1.pt $WEIGHTS_DST/port_localizer_dinov2.pt" >&2
  echo "  cp data/outputs/loc_stats_dinov3_gz_v1.npz $WEIGHTS_DST/port_localizer_stats.npz" >&2
  exit 1
fi

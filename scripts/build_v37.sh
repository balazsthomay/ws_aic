#!/usr/bin/env bash
# v37: OnlineIK with localizer retrained on Gazebo demos.
set -euo pipefail
...
TAG="${TAG:-v37}"
DOCKERFILE="${DOCKERFILE:-$SRC/docker/aic_model/Dockerfile.v37}"
...
if [[ ! -f "$WEIGHTS_DST/port_localizer_dinov2.pt" || ! -f "$WEIGHTS_DST/port_localizer_stats.npz" ]]; then
  ...
  echo "  cp data/outputs/loc_v37.pt $WEIGHTS_DST/port_localizer_dinov2.pt" >&2
  echo "  cp data/outputs/loc_v37_stats.npz $WEIGHTS_DST/port_localizer_stats.npz" >&2
  exit 1
fi

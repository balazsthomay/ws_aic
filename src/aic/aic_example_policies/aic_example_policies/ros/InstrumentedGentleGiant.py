"""GentleGiant clone instrumented with HTTP egress to AIC_DIAG_URL.

Same shape as InstrumentedSpeedDemon but with low stiffness + high damping
to avoid the Tier 2 force penalty SpeedDemon incurs by design.
"""
...
_diag("module_imported", policy="InstrumentedGentleGiant")


class InstrumentedGentleGiant(Policy):
    ...
    def insert_cable(
        ...
        self.get_logger().info(f"InstrumentedGentleGiant.insert_cable() task: {task}")
        send_feedback("gentle baseline policy")
